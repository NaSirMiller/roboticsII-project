  ...
  explore_lite_msgs__msg__ExploreStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    explore_lite_msgs__msg__ExploreStatus * data =
      (explore_lite_msgs__msg__ExploreStatus *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!explore_lite_msgs__msg__ExploreStatus__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          explore_lite_msgs__msg__ExploreStatus__fini(&data[i]);
        }
        free(data);
        return false;
      }
