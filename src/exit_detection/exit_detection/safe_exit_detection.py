...
class SafeExitDetectionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('color_low', [346, 33, 76]) # pink, hsv
        self.declare_parameter('color_high', [354, 20, 100])
    ...
    def camera_callback(self, rgb_msg, points_msg):
        ...
        if np.any(np.isnan(center_points)) or np.any(np.isinf(center_points)):
        # or Z > 0.4 or Y > 0.25:
            return
        try:
