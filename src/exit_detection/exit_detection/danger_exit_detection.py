...
class DangerExitDetectionNode(Node):
    def __init__(self):
        ...
        self.pub_danger_exit_pose = self.create_publisher(PoseStamped, 'move_base_simple/goal', 10)
    ...
    def camera_callback(self, rgb_msg, points_msg):
        ...
        try: # Transform center point from the camera frame to the world frame
            ...
            detected_danger_exit_pose.pose.orientation.w = 1.0
        ...
        if not self.goal_sent or elapsed > 5.0:  # Only send every 5 seconds
            self.pub_danger_exit_pose.publish(detected_danger_exit_pose)
